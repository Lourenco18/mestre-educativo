...
<thead>
  <tr>
  ';
if ($tabela == 'carro' || $tabela == "stand" || $tabela == "marca" || $tabela == "empresa" || $tabela == "servico" || $tabela == "user") {
  echo '<th>Foto</th>';
}

echo '<th>' . ucfirst($tabela) . '</th>';
...
if (isset($arrresults[$k]['descricao'])) {
...
foreach ($arrresults as $k => $v) {
  ...
  //fotos
  if ($tabela == 'carro') {
    $arrfoto = my_query('SELECT foto FROM fotos where ativo = 1 and principal = 1 and id_car =' . $v['id_carro']);
    $foto = $arrfoto[0]['foto'];
    $src = $arrConfig['url_fotos_upload'] . '/' . $tabela . '/' . $foto . '"';
    echo '<td><img class="icons"  src="' . $src . ' height="100" width="100"></td>';
  } else {



    if ($tabela == "stand" || $tabela == "marca" || $tabela == "empresa" || $tabela == "servico" || $tabela == "user") {


      $arrfoto = my_query("SELECT foto_$tabela from $tabela where id_$tabela = $id");
      $foto = $arrfoto[0]['foto_' . $tabela];
      $src = $arrConfig['url_fotos_upload'] . '/' . $tabela . '/' . $foto . '"';
      echo '<td><img class="icons"  src="' . $src . ' height="100" width="100"></td>';
    } else {
  ...
  if ($tabela != 'testemunho' && $tabela != 'user') {
  ...
  if ($tabela == 'user') {
    if ($v['id_cargo'] != $arrConfig['super_admin']) {
      echo '<button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#modalTopremove_'.$id.'" title="Remover"><i class="bx bx-trash"></i></button>';
    }
  } else {
    echo '<button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#modalTopremove_'.$id.'" title="Remover"><i class="bx bx-trash"></i></button>';
  }
